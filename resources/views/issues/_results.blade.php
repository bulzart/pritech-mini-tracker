{{--
    The issue results region: the table of issues with pagination, or the empty
    state. Rendered inside the live region on the issues index, and returned on
    its own for the AJAX search/filter swap (IssueController@index on an XHR
    request — public/js/issues-index.js).
    Expects: $issues (LengthAwarePaginator), $filters (search/status/priority/tag).
--}}
@if ($issues->isEmpty())
    <div class="card empty-state">
        <h2>No issues found</h2>
        @if (filled($filters['search'] ?? '') || filled($filters['status'] ?? '') || filled($filters['priority'] ?? '') || filled($filters['tag'] ?? ''))
            <p>No issues match your search and filters.</p>
            <a href="{{ route('issues.index') }}" class="button">Clear search and filters</a>
        @else
